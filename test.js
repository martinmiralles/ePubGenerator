...
fetch("/../json/re6_teacher/content_json.json")
  .then(function(res) {
    return res.json();
  })
  .then(function(data) {
...
function createXHTMLZipFolder(data) {
  ...
  for (var key in data) {
    ...
    for (let i = 0; i < data[key].length; i++) {
      ...
      doc.getElementById("app").appendChild(badge1);
      ...
      // console.log("\n");
      console.log(JSON.parse(JSON.stringify(xhtml.outerHTML)));

      //BELOW: For the .xhtml file(s) creation AND add them to the "xhtml" folder
      ops.folder("xhtml").file(key + ".xhtml", pretty(xhtml.outerHTML));
    }
